#!/usr/bin/env python3
"""
Merge plan-dataamna.json and plan-dataismail.json into the main plan-data.json file.
...
def main():
    ...
    print("Merging plan-dataamna.json and plan-dataismail.json into plan-data.json")
    print("=" * 80)
    print()
    ...
    amna_data = load_json_file('plan-dataamna.json')
    ismail_data = load_json_file('plan-dataismail.json')
    
    # Check if at least one source file exists
    if not amna_data and not ismail_data:
        print("\n❌ Error: Neither plan-dataamna.json nor plan-dataismail.json found!")
        print("   Please create these files before running the merge.")
        return False
    
    if amna_data:
        print(f"✅ plan-dataamna.json loaded: {len(amna_data.get('inspectionData', []))} inspection entries")
    else:
        print("⚠️  plan-dataamna.json not found, skipping...")
    
    if ismail_data:
        print(f"✅ plan-dataismail.json loaded: {len(ismail_data.get('inspectionData', []))} inspection entries")
    else:
        print("⚠️  plan-dataismail.json not found, skipping...")
    ...
    # Merge amna data
    if amna_data:
        stats = merge_source_into_main(main_data, amna_data, "plan-dataamna.json")
        for key in total_stats:
            total_stats[key] += stats[key]
    
    # Merge ismail data
    if ismail_data:
        stats = merge_source_into_main(main_data, ismail_data, "plan-dataismail.json")
        for key in total_stats:
            total_stats[key] += stats[key]
