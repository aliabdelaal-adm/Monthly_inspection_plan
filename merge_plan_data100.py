#!/usr/bin/env python3
"""
Merge plan-data100.json into the main plan-data.json file.
This script safely merges inspection data while replacing the inspectors list
with the 9 official inspectors from plan-data100.json.
"""
...
def merge_list_data(main_list, source_list, id_field='id'):
    """Merge list data (areas, shops) avoiding duplicates by ID."""
    if not source_list:
        return main_list, 0
...
def main():
    ...
    print("Merging plan-data100.json into plan-data.json")
    print("⚠️  This will REPLACE the inspectors list with the 9 official inspectors")
    print()
    
    # Load files
    print("📂 Loading files...")
    main_data = load_json_file('plan-data.json')
    source_data = load_json_file('plan-data100.json')
    ...
    print(f"   Current inspectors: {len(main_data.get('inspectors', []))}")
    print(f"✅ Source file loaded: {len(source_data.get('inspectionData', []))} inspection entries")
    print(f"   Official inspectors: {len(source_data.get('inspectors', []))}")
    print()
    ...
    # REPLACE inspectors list with the one from source (9 official inspectors)
    old_inspector_count = len(main_data.get('inspectors', []))
    merged_data['inspectors'] = source_data.get('inspectors', [])
    new_inspector_count = len(merged_data['inspectors'])
    print(f"🔧 Replacing inspectors list: {old_inspector_count} → {new_inspector_count}")
    ...
    print(f"   👥 Inspectors replaced: {old_inspector_count} → {new_inspector_count}")
