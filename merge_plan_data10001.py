#!/usr/bin/env python3
"""
Merge plan-data10001.json into the main plan-data.json file.
...
def main():
    ...
    print("Merging plan-data10001.json into plan-data.json")
    print()
    
    # Load files
    print("📂 Loading files...")
    main_data = load_json_file('plan-data.json')
    source_data = load_json_file('plan-data10001.json')
    ...
    print(f"      - Inspection entries: {len(merged_data.get('inspectionData', []))}")
    print(f"      - Inspectors: {len(merged_data.get('inspectors', []))}")
    print(f"      - Areas: {len(merged_data.get('areas', []))}")
    print(f"      - Shops: {len(merged_data.get('shops', []))}")
    print(f"      - Last update: {merged_data.get('lastUpdate')}")
    print()
    print("✅ The merged data is ready to be read by index.html")
