#!/usr/bin/env python3
"""
Merge plan-dataddd.json and plan-datazz.json into the main plan-data.json file.
...
def main():
    ...
    print("Merging plan-dataddd.json and plan-datazz.json into plan-data.json")
    print("=" * 80)
    print()
    ...
    source_ddd = load_json_file('plan-dataddd.json')
    source_zz = load_json_file('plan-datazz.json')
    
    if not main_data or not source_ddd or not source_zz:
        print("❌ Failed to load required files!")
        return False
    
    print(f"✅ Main file loaded: {len(main_data.get('inspectionData', []))} inspection entries")
    print(f"✅ Source DDD loaded: {len(source_ddd.get('inspectionData', []))} inspection entries")
    print(f"✅ Source ZZ loaded: {len(source_zz.get('inspectionData', []))} inspection entries")
    print()
    ...
    print("🔄 Merging data from plan-dataddd.json...")
    merged_data = main_data.copy()
    
    # Merge inspection data from DDD
    merged_inspections, new_from_ddd, conflicts_ddd = merge_inspection_data(main_data, source_ddd, "plan-dataddd.json")
    merged_data['inspectionData'] = merged_inspections
    
    # Merge inspection data from ZZ
    print("🔄 Merging data from plan-datazz.json...")
    temp_data = {'inspectionData': merged_inspections}
    merged_inspections, new_from_zz, conflicts_zz = merge_inspection_data(temp_data, source_zz, "plan-datazz.json")
    merged_data['inspectionData'] = merged_inspections
    
    # Report conflicts
    all_conflicts = conflicts_ddd + conflicts_zz
    if all_conflicts:
        print()
    ...
    merged_inspectors, new_insp_ddd = merge_list_data(
        main_data.get('inspectors', []), 
        source_ddd.get('inspectors', [])
    )
    merged_inspectors, new_insp_zz = merge_list_data(
        merged_inspectors, 
        source_zz.get('inspectors', [])
    )
    merged_data['inspectors'] = merged_inspectors
    
    # Merge areas from both sources
    print("🔄 Merging areas...")
    merged_areas, new_area_ddd = merge_list_data(
        main_data.get('areas', []), 
        source_ddd.get('areas', [])
    )
    merged_areas, new_area_zz = merge_list_data(
        merged_areas, 
        source_zz.get('areas', [])
    )
    ...
    print("🔄 Merging shops...")
    merged_shops, new_shop_ddd = merge_list_data(
        main_data.get('shops', []), 
        source_ddd.get('shops', [])
    )
    merged_shops, new_shop_zz = merge_list_data(
        merged_shops, 
        source_zz.get('shops', [])
    )
    ...
    merged_bell, new_notif_ddd = merge_bell_notes(main_data, source_ddd)
    merged_bell, new_notif_zz = merge_bell_notes({'bellNotes': merged_bell}, source_zz)
    ...
    print()
    print("📊 Merge Summary:")
    print(f"   From plan-dataddd.json:")
    print(f"      📝 New inspection entries: {new_from_ddd}")
    print(f"      👥 New inspectors: {new_insp_ddd}")
    print(f"      🏘️  New areas: {new_area_ddd}")
    print(f"      🏪 New shops: {new_shop_ddd}")
    print(f"      🔔 New notifications: {new_notif_ddd}")
    print()
    print(f"   From plan-datazz.json:")
    print(f"      📝 New inspection entries: {new_from_zz}")
    print(f"      👥 New inspectors: {new_insp_zz}")
    print(f"      🏘️  New areas: {new_area_zz}")
    print(f"      🏪 New shops: {new_shop_zz}")
    print(f"      🔔 New notifications: {new_notif_zz}")
    print()
