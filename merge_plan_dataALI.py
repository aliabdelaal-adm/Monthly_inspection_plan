#!/usr/bin/env python3
"""
Merge plan-dataALI.json into the main plan-data.json file.
...
def main():
    ...
    print("Merging plan-dataALI.json into plan-data.json")
    print()
    
    # Load files
    print("📂 Loading files...")
    main_data = load_json_file('plan-data.json')
    source_data = load_json_file('plan-dataALI.json')
    ...
    # Validate for duplicate shop assignments - check if merge introduces NEW duplicates
    print("🔍 Validating inspection data for duplicate shops...")
    
    # Check duplicates in main data before merge
    is_valid_main, duplicates_main = validate_shop_duplicates(main_data['inspectionData'])
    main_dup_count = len(duplicates_main)
    
    # Check duplicates after merge
    is_valid_merged, duplicates_merged = validate_shop_duplicates(merged_data['inspectionData'])
    merged_dup_count = len(duplicates_merged)
    
    # Only fail if the merge introduced NEW duplicates beyond what already existed
    if merged_dup_count > main_dup_count:
        new_duplicates = merged_dup_count - main_dup_count
        print()
        print(f"❌ خطأ: الدمج قد أضاف {new_duplicates} تكرارات جديدة للمحلات!")
        print(f"❌ Error: Merge would introduce {new_duplicates} new duplicate shop assignments!")
        print()
        print("🔔 تفاصيل التكرارات الجديدة / New Duplicate Details:")
        print("=" * 80)
        
        # Show only the new duplicates (simplified - showing all for now)
        for dup in duplicates_merged[:5]:  # Show first 5 as sample
    ...
    if main_dup_count > 0:
        print(f"⚠️  Note: {main_dup_count} pre-existing duplicate shop assignments detected (not blocking merge)")
    print("✅ Validation passed: No new duplicate shops introduced by merge")
