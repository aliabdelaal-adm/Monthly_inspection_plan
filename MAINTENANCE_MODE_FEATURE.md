# نظام وضع الصيانة - Maintenance Mode System

## 📋 نظرة عامة - Overview

تم تطوير نظام وضع الصيانة المتقدم للتحكم في عرض النظام للمستخدمين أثناء إجراء التحديثات والصيانة.

## ✨ الميزات الرئيسية - Key Features

### 1. 🛡️ وضع الصيانة المتزامن عبر GitHub
- يتم حفظ حالة وضع الصيانة على GitHub لمزامنتها عبر جميع الأجهزة
- عند تفعيل وضع الصيانة، يرى جميع المستخدمين (عدا المطور) رسالة التحديث **على جميع الأجهزة**
- المطور فقط يمكنه تجاوز وضع الصيانة والوصول للنظام
- يتم التحقق من حالة الصيانة تلقائياً كل 10 ثوان

### 2. 🔐 التحكم الكامل للمطور
- يمتلك المطور صلاحيات كاملة للتحكم في وضع الصيانة
- يمكن تفعيل/إلغاء وضع الصيانة يدوياً من لوحة التحكم
- **وضع الصيانة يبقى مفعلاً حتى بعد خروج المطور من النظام**
- يجب إلغاء وضع الصيانة يدوياً باستخدام زر "إلغاء وضع الصيانة للجميع"

### 3. 🎨 واجهة مستخدم جميلة
- تصميم عصري وجذاب لشاشة الصيانة
- رسوم متحركة سلسة ومريحة للعين
- رسائل واضحة باللغة العربية

## 🔧 كيفية الاستخدام - How to Use

### للمطور - For Developer:

#### 1. تسجيل الدخول كمطور
```
1. اختر "المطور" من قائمة تسجيل الدخول
2. أدخل كلمة السر
3. انقر "دخول المطور"
```

#### 2. تفعيل وضع الصيانة للجميع
```javascript
// في لوحة التحكم، انقر على:
🔒 تفعيل وضع الصيانة للجميع
```
- سيرى جميع المستخدمين (عدا المطور) رسالة التحديث **على جميع الأجهزة**
- تظهر الرسالة تلقائياً خلال 10-30 ثانية على الأجهزة الأخرى
- المطور يستطيع الاستمرار في العمل بشكل طبيعي

#### 3. إلغاء وضع الصيانة للجميع
```javascript
// في لوحة التحكم، انقر على:
✅ إلغاء وضع الصيانة للجميع
```
- يعود النظام للعمل بشكل طبيعي للجميع

#### 4. الخروج من حساب المطور
```
انقر "إغلاق المطور"
```
- **وضع الصيانة يبقى مفعّلاً حتى بعد تسجيل الخروج**
- يجب إلغاء وضع الصيانة يدوياً قبل الخروج (إذا أردت)
- أو يمكنك تسجيل الدخول لاحقاً وإلغاء وضع الصيانة

### للمستخدمين العاديين - For Regular Users:

عند تفعيل وضع الصيانة:
- سترى رسالة: "الزملاء الأعزاء - جاري تحديث البيانات - شكراً على الانتظار"
- لن تتمكن من الوصول للنظام حتى يتم إلغاء وضع الصيانة
- انتظر حتى يقوم المطور بإكمال التحديثات

## 💻 الوظائف البرمجية - Code Functions

### 1. `enableMaintenanceModeForAll()`
**الوصف**: تفعيل وضع الصيانة لجميع المستخدمين (عدا المطور) على جميع الأجهزة
**الاستخدام**: 
```javascript
await enableMaintenanceModeForAll(); // async function
```
**ملاحظة**: يتم حفظ الحالة على GitHub لمزامنة جميع الأجهزة

### 2. `disableMaintenanceModeForAll()`
**الوصف**: إلغاء وضع الصيانة للجميع على جميع الأجهزة
**الاستخدام**: 
```javascript
await disableMaintenanceModeForAll(); // async function
```
**ملاحظة**: يتم تحديث الحالة على GitHub لإلغائها من جميع الأجهزة

### 3. `checkMaintenanceMode()`
**الوصف**: التحقق من حالة وضع الصيانة من GitHub
**الاستخدام**: يتم استدعاؤها تلقائياً كل 10 ثوان
**ملاحظة**: تقرأ الحالة من GitHub لضمان التزامن

### 4. `startMaintenanceStatusChecker()`
**الوصف**: بدء الفحص الدوري لحالة الصيانة
**الاستخدام**: يتم استدعاؤها تلقائياً عند تحميل الصفحة
**الفترة الزمنية**: كل 10 ثوان

### 5. `saveMaintenanceStatusToGitHub(isEnabled, messages)`
**الوصف**: حفظ حالة الصيانة على GitHub
**المدخلات**: 
- `isEnabled`: true لتفعيل، false لإلغاء
- `messages`: مصفوفة الرسائل
**ملاحظة**: للمطور فقط

### 6. `loadMaintenanceStatusFromGitHub()`
**الوصف**: تحميل حالة الصيانة من GitHub
**الإرجاع**: كائن يحتوي على حالة الصيانة
**ملاحظة**: متاح لجميع المستخدمين

### 7. `showMaintenanceMode(issues)`
**الوصف**: عرض شاشة الصيانة مع رسائل مخصصة
**المدخلات**: `issues` - مصفوفة من الرسائل
**مثال**: 
```javascript
showMaintenanceMode(['جاري تحديث النظام', 'يقوم المطور بإجراء تعديلات']);
```

### 8. `hideMaintenanceMode()`
**الوصف**: إخفاء شاشة الصيانة
**الاستخدام**: 
```javascript
hideMaintenanceMode();
```

## 🔐 التخزين - Storage

### 1. GitHub (التخزين المركزي):
**الملف**: `maintenance-status.json`
```json
{
  "isMaintenanceMode": true,
  "lastUpdated": "2024-01-15T10:30:00Z",
  "updatedBy": "المطور",
  "messages": ["جاري تحديث النظام", "..."]
}
```

### 2. localStorage (التخزين المحلي - للنسخ الاحتياطي):
**المفتاح**: `'systemMaintenanceMode'`
**القيم الممكنة**:
- `'true'` - وضع الصيانة مفعّل
- `null` أو غير موجود - وضع الصيانة غير مفعّل

## 🎯 سيناريوهات الاستخدام - Use Cases

### السيناريو 1: صيانة طارئة
```
1. المطور يسجل دخول
2. المطور ينقر "تفعيل وضع الصيانة للجميع"
3. جميع المستخدمين يرون رسالة الصيانة
4. المطور يقوم بالتعديلات اللازمة
5. المطور ينقر "إلغاء وضع الصيانة للجميع"
6. الجميع يمكنهم استخدام النظام
```

### السيناريو 2: تحديث مجدول
```
1. المطور يسجل دخول قبل بدء الصيانة
2. المطور يفعّل وضع الصيانة
3. المطور يقوم بالتحديثات
4. عند الانتهاء، المطور يخرج من النظام
5. وضع الصيانة يُلغى تلقائياً
```

## 📱 التوافق - Compatibility

- ✅ يعمل على جميع المتصفحات الحديثة
- ✅ يدعم الهواتف المحمولة (Mobile)
- ✅ يدعم الأجهزة اللوحية (Tablets)
- ✅ يدعم أجهزة الكمبيوتر (Desktop)

## 🔍 استكشاف الأخطاء - Troubleshooting

### المشكلة: وضع الصيانة لا يظهر
**الحل**: تأكد من تفعيله من لوحة تحكم المطور أو تحقق من قيمة localStorage

### المشكلة: المطور يرى شاشة الصيانة
**الحل**: تأكد من تسجيل الدخول بنجاح كمطور

### المشكلة: وضع الصيانة لا يزال مفعلاً بعد خروج المطور
**هذا ليس خطأ!** وضع الصيانة مصمم للبقاء مفعلاً حتى بعد خروج المطور. يجب استخدام زر "إلغاء وضع الصيانة للجميع" لإيقافه يدوياً

## 📝 ملاحظات مهمة - Important Notes

1. ✅ **وضع الصيانة متزامن عبر GitHub** - يظهر على جميع الأجهزة تلقائياً
2. ⏰ **الفحص التلقائي كل 10 ثوان** - يتم التحقق من الحالة دورياً
3. 🔒 **المطور فقط** - يمكنه دائماً الوصول للنظام حتى في وضع الصيانة
4. ⚡ **التزامن السريع** - تظهر الرسالة خلال 10-30 ثانية على جميع الأجهزة
5. 🌐 **يتطلب اتصال بالإنترنت** - لكي يعمل التزامن عبر GitHub

## 🚀 التطويرات - Development History

### الإصدار 2.0 (يناير 2025) ✅
- [x] ✅ **مزامنة حالة الصيانة عبر GitHub لجميع المستخدمين**
- [x] ✅ **الفحص التلقائي كل 10 ثوان**
- [x] ✅ **عرض الرسالة على جميع الأجهزة تلقائياً**

### التطويرات المستقبلية المحتملة:
- [ ] جدولة تلقائية لوضع الصيانة
- [ ] إشعارات مسبقة للمستخدمين قبل بدء الصيانة
- [ ] تخصيص رسائل الصيانة لكل مستخدم
- [ ] لوحة تحكم لمراقبة المستخدمين النشطين

---

**تم التطوير بواسطة © المطور د. علي عبدالعال**
